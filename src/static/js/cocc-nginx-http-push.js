...
function coccNginxHttpPush() {			
    ...
    var cocc_nip = (typeof(cocc_is_ssl) != 'undefined' && cocc_is_ssl?"https":"http")+"://"+cocc_ip;
	...
	cocc_NginxHttpPushFrameHtml += '					try{window.parent.sonAccept(""+jsondata.pushstr+"");}catch(e){}';
	//cocc_NginxHttpPushFrameHtml += '				}';
	...
	cocc_NginxHttpPushFrameHtml += '			var url = "'+cocc_nip+'/publicapi/agentpull/"+teamidentity+"-"+agentno+"-"+pwd+"?_="+urlrandom+"&callback=coccEvents&etag="+etag+"&since="+since+"";';
	cocc_NginxHttpPushFrameHtml += '		} else {';
	cocc_NginxHttpPushFrameHtml += '			var url = "'+cocc_nip+'/publicapi/agentpull/"+teamidentity+"-"+pwd+"?_="+urlrandom+"&callback=coccEvents&etag="+etag+"&since="+since+"";';	
	cocc_NginxHttpPushFrameHtml += '		}';
...
function coccNginxHttpPushNAPI() {
	var cocc_nip = (typeof(cocc_is_ssl) != 'undefined' && cocc_is_ssl?"https":"http")+"://"+cocc_ip;
	$.getJSON(cocc_nip + "/getccdate/getdate?callback=?", {}, function(json) {
		try {
			cocc_lastmodified = json.ccdate;
			coccNginxHttpPush();
		} catch (e) {
